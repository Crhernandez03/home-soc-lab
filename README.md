...
- **Network-layer detection** — Suricata (NIDS) on the defender host
  inspects raw traffic, catching activity like port scans that
  host-based log monitoring alone would miss.
...
6. **(Optional) Add network-layer detection** — install Suricata on
   the defender host and wire its `eve.json` log into the Wazuh agent
